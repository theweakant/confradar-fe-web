// components/(user)/workspace/organizer/ManageConference/ConferenceForm/research/ResearchConferenceStepForm.tsx
"use client";
...
import { RESEARCH_STEP_LABELS, RESEARCH_MAX_STEP } from "@/components/molecules/Conference/ConferenceStep/constants";
...
export default function ResearchConferenceStepForm({
  ...
  // === LOAD EXISTING DATA (ALWAYS CALL HOOK, CONDITIONAL LOGIC INSIDE) ===
  const { isLoading: isConferenceLoading } = useResearchConferenceData({
    ...
    (field: string) => {
      switch (field) {
        ...
        case "ticketSaleStart":
          validate(field, () => validateTicketSaleStart(basicForm.ticketSaleStart, basicForm.startDate));
          break;
        case "ticketSaleDuration":
          if (basicForm.ticketSaleDuration != null && basicForm.ticketSaleStart && basicForm.startDate) {
            validate(field, () =>
              validateTicketSaleDuration(
  ...
  const isLoading =
    mode === "edit" &&
    (isConferenceLoading || isCategoriesLoading || isRoomsLoading || isCitiesLoading || isRankingLoading);
  ...
  return (
    <div className="max-w-5xl mx-auto p-6">
      <PageHeader
        title={mode === "create" ? "Tạo hội thảo nghiên cứu mới" : "Chỉnh sửa hội thảo nghiên cứu"}
        description={
          mode === "create"
      ...
      {currentStep === 2 && (
        <StepContainer stepNumber={2} title="Chi tiết nghiên cứu" isCompleted={isStepCompleted(2)}>
          ...
          <FlexibleNavigationButtons
            ...
            onNext={mode === "edit" ? handleNextStep : undefined}
            onSubmit={mode === "create" ? handleResearchDetailSubmit : undefined}
            onUpdate={mode === "edit" ? handleUpdateCurrentStep : undefined}
          />
        </StepContainer>
      )}
      ...
      {currentStep === 3 && (
        <StepContainer stepNumber={3} title="Timeline & Giai đoạn" isCompleted={isStepCompleted(3)}>
          ...
          <FlexibleNavigationButtons
            ...
            onPrevious={handlePreviousStep}
            onNext={mode === "edit" ? handleNextStep : undefined}
            onSubmit={mode === "create" ? handleTimelineSubmit : undefined}
            onUpdate={mode === "edit" ? handleUpdateCurrentStep : undefined}
          />
        </StepContainer>
      )}
      ...
      {currentStep === 7 && (
        <StepContainer stepNumber={7} title="Tài liệu & Xếp hạng (Tùy chọn)" isCompleted={isStepCompleted(7)}>
          ...
          <FlexibleNavigationButtons
            ...
            isOptionalStep={true}
            isSkippable={
              researchMaterials.length === 0 && rankingFiles.length === 0 && rankingReferences.length === 0
            }
            onPrevious={handlePreviousStep}
            onNext={handleNextStep}
            onSubmit={mode === "create" ? handleMaterialsSubmit : undefined}
            onUpdate={mode === "edit" ? handleUpdateCurrentStep : undefined}
          />
        </StepContainer>
      )}
      ...
      {currentStep === 8 && (
        <StepContainer stepNumber={8} title="Media (Tùy chọn)" isCompleted={isStepCompleted(8)}>
          <MediaForm mediaList={mediaList} onMediaListChange={setMediaList} onRemoveMedia={deleteTracking.trackDeletedMedia} />
          <FlexibleNavigationButtons
            currentStep={8}
            maxStep={RESEARCH_MAX_STEP}
            isSubmitting={isSubmitting}
