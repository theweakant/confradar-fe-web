...
export function HoverRevealSquare({
  ...
  const animationDuration = prefersReducedMotion ? "duration-0" : "duration-500"
    ...
    <div className={cn("relative", className)}>
              ...
              <div className="w-[85%] aspect-square rounded-full bg-[#0066FF] flex items-center justify-center p-8 shadow-2xl">
                <div className="text-center space-y-2">
                  {hoverInfo.map((item, index) => (
                    <p key={index} className="text-sm md:text-base lg:text-lg font-semibold text-white">
                      {item}
                    </p>
                  ))}
                </div>
      ...
      {(detailNumber || detailTitle || detailDescription) && (
        ...
        >
          ...
          {detailNumber && <p className="text-xs tracking-[0.2em] uppercase text-white/40 mb-2">{detailNumber}</p>}

          {detailTitle && <h4 className="text-xl md:text-2xl font-bold text-[#FF4500] mb-3">{detailTitle}</h4>}
