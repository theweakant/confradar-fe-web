...
export default function FavoriteConferences() {
  ...
  const filterOptions = [
    { id: "most-popular", label: "Phổ biến nhất", active: activeFilter === "most-popular" },
    { id: "start-date-desc", label: "Ngày bắt đầu (Mới nhất)", active: activeFilter === "start-date-desc" },
    { id: "start-date-asc", label: "Ngày bắt đầu (Cũ nhất)", active: activeFilter === "start-date-asc" },
    { id: "alphabetical", label: "Theo bảng chữ cái", active: activeFilter === "alphabetical" },
  ...
  useEffect(() => {
    ...
    const sorted = [...favouriteConferences].sort((a, b) => {
      switch (activeFilter) {
        ...
        case "start-date-asc":
          return new Date(a.conferenceStartDate || '').getTime() - new Date(b.conferenceStartDate || '').getTime();
        case "alphabetical":
          return (a.conferenceName || '').localeCompare(b.conferenceName || '');
        case "favorite-date":
          return new Date(b.favouriteCreatedAt || '').getTime() - new Date(a.favouriteCreatedAt || '').getTime();
  ...
  const handleViewDetail = (conferenceId: string, isResearchConference?: boolean) => {
        ...
        {sortedConferences.length === 0 ? (
          ...
          <div className="space-y-6">
            {sortedConferences.map((conference) => (
              <Card key={conference.conferenceId} className="overflow-hidden hover:shadow-lg transition-shadow bg-gray-800 border-gray-700">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                      <div className="flex-1 space-y-3">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pt-2">
                          <div className="flex items-center gap-2 text-sm text-gray-400">
                            <span>Yêu thích từ: {formatDate(conference.favouriteCreatedAt)}</span>
                          </div>
                          <div className="flex gap-2">
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => handleRemoveFavorite(conference.conferenceId)}
                            ...
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => handleViewDetail(conference.conferenceId, conference.isResearchConference)}
                  ...
                  <div className="lg:w-80 lg:flex-shrink-0">
                    {conference.bannerImageUrl ? (
                      <div
                        ...
                        style={{ backgroundImage: `url(${conference.bannerImageUrl})` }}
                      />
                    ) : (
