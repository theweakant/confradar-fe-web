...
export function ResearchSingleSessionForm({
  ...
  const normalizeToISO = (timeInput: string | undefined, fallbackISO: string): string => {
    if (!timeInput) return fallbackISO;

    if (timeInput.includes('T')) {
      const d = new Date(timeInput);
      return isNaN(d.getTime()) ? fallbackISO : timeInput;
    }

    const dateOnly = date.includes('T') ? date.split('T')[0] : date;
    const isoString = `${dateOnly}T${timeInput}`;
    const d = new Date(isoString);
  ...
  const calculateTimeRangeFromSession = (session?: ResearchSession): number => {
    ...
    const startISO = normalizeToISO(session.startTime, slotStartTime);
    const endISO = normalizeToISO(session.endTime, slotEndTime);
  ...
  const [formData, setFormData] = useState({
    ...
    selectedStartTime: normalizeToISO(initialSession?.startTime, slotStartTime),
    timeRange: calculateTimeRangeFromSession(initialSession) || 1,
    sessionMedias: initialSession?.sessionMedias || ([] as SessionMedia[]),
  });

  const [calculatedEndTime, setCalculatedEndTime] = useState(
    normalizeToISO(initialSession?.endTime, slotEndTime)
  );
  ...
  useEffect(() => {
    ...
    if (proposedEnd > maxEnd) {
      ...
      setFormData((prev) => ({ 
        ...prev, 
        timeRange: Math.max(0.5, Math.floor(maxHours * 2) / 2) 
      }));
    } else {
  ...
  useEffect(() => {
    if (initialSession) {
      const safeStartTime = normalizeToISO(initialSession.startTime, slotStartTime);
      const safeEndTime = normalizeToISO(initialSession.endTime, slotEndTime);
  ...
  const handleSubmit = () => {
    ...
    if (!formData.selectedStartTime || !calculatedEndTime) {
    ...
    if (isNaN(startDate.getTime()) || isNaN(endDate.getTime())) {
      toast.error("Thời gian không đúng định dạng!");
      return;
    }

    const startTimeStr = extractTimeOnly(formData.selectedStartTime);
    const endTimeStr = extractTimeOnly(calculatedEndTime);

    if (startTimeStr === "00:00:00" && startDate.getHours() !== 0) {
      toast.error("Lỗi: Thời gian bắt đầu không hợp lệ!");
      return;
    }

    if (endTimeStr === "00:00:00" && endDate.getHours() !== 0) {
      toast.error("Lỗi: Thời gian kết thúc không hợp lệ!");
      return;
    }
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="bg-gradient-to-br from-purple-50 to-purple-100 border border-purple-200 rounded-lg p-3">
          ...
          <div className="text-sm font-semibold text-gray-900">{roomDisplayName}</div>
          {roomNumber && <div className="text-xs text-gray-600 mt-0.5">Số: {roomNumber}</div>}
        </div>
      </div>
