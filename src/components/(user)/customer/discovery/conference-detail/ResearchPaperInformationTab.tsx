...
const ResearchPaperInformationTab: React.FC<
  ResearchPaperInformationTabProps
> = ({ conference, formatDate, formatTime }) => {
  return (
    <div>
          ...
          <div className="bg-white/20 backdrop-blur-md rounded-lg p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {conference.researchPhase.fullPaperStartDate &&
                conference.researchPhase.fullPaperEndDate && (
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-white/80 text-sm">
                        ...
                        -{" "}
                        {formatDate(conference.researchPhase.fullPaperEndDate)}
                      </p>
                    </div>
                  </div>
                )}

              {conference.researchPhase.reviewStartDate &&
                conference.researchPhase.reviewEndDate && (
                  <div className="flex items-center gap-3">
                    <Clock className="w-5 h-5 text-yellow-400" />
                    <div>
                      <p className="text-white font-medium">Review Period</p>
                      <p className="text-white/80 text-sm">
                        {formatDate(conference.researchPhase.reviewStartDate)} -{" "}
                        {formatDate(conference.researchPhase.reviewEndDate)}
                      </p>
                    </div>
                  </div>
                )}

              {conference.researchPhase.reviseStartDate &&
                conference.researchPhase.reviseEndDate && (
                  <div className="flex items-center gap-3">
                    <FileText className="w-5 h-5 text-orange-400" />
                    <div>
                      <p className="text-white font-medium">Revision Period</p>
                      <p className="text-white/80 text-sm">
                        {formatDate(conference.researchPhase.reviseStartDate)} -{" "}
                        {formatDate(conference.researchPhase.reviseEndDate)}
                      </p>
                    </div>
                  </div>
                )}
            ...
            <div className="mt-4 pt-4 border-t border-white/20">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-white font-medium">
                    {conference.researchPhase.researchConferencePhaseId ||
                      "Chưa có ID phase"}
                  </p>
                </div>
                <div>
                  ...
                  <p className="text-white font-medium">
                    {conference.researchPhase.conferenceId ||
                      "Chưa có Conference ID"}
                  </p>
                </div>
                <div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <h4 className="text-white font-medium">
                        {material.fileName || "Download Material"}
                      </h4>
      ...
      {conference.researchSessions &&
        conference.researchSessions.length > 0 && (
          <div className="mb-8">
            <h3 className="text-xl font-semibold text-white mb-4">
              Research Sessions
            </h3>
                ...
                .map((session) => (
                  ...
                  >
                    <h4 className="text-xl font-bold text-white mb-2">
                      {session.title || "Chưa có tiêu đề session"}
                    </h4>
                    {session.description && (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                      ...
                      <div>
                        ...
                        <p className="text-white font-medium">
                          {session.conferenceId || "Chưa có Conference ID"}
                        </p>
                      </div>
                      <div>
                        <span className="text-white/70 text-sm">Room ID:</span>
                        <p className="text-white font-medium">
                          {session.roomId || "Chưa có Room ID"}
                        </p>
                      </div>
                      {session.room && (
                        <div>
                          ...
                          <p className="text-white font-medium">
                            ...
                            {session.room.destinationId &&
                              ` (Destination: ${session.room.destinationId})`}
                          </p>
                        </div>
                      )}
                    </div>
                        ...
                        <div className="mt-3">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                            {session.sessionMedia.map((media, index) => (
                              <div
                                key={media.conferenceSessionMediaId || index}
                                className="bg-white/10 rounded p-2"
                              >
                                <div className="text-white/70 text-xs mb-1">
                                  Media ID:{" "}
                                  {media.conferenceSessionMediaId || "N/A"}
                                </div>
      ...
      {/* Empty State */}
      {!conference.researchPhase &&
        (!conference.rankingFileUrls ||
          conference.rankingFileUrls.length === 0) &&
        (!conference.materialDownloads ||
          conference.materialDownloads.length === 0) &&
        (!conference.rankingReferenceUrls ||
          conference.rankingReferenceUrls.length === 0) &&
