...
export function ResearchSingleSessionForm({
  ...
  const getSafeTime = (time: string | undefined, fallback: string): string => {
    ...
    console.log("🔄 getSafeTime:", { original: time, combined, valid: !isNaN(d.getTime()) });

    return isNaN(d.getTime()) ? fallback : combined;
  };
  ...
  const calculateTimeRangeFromSession = (session?: ResearchSession): number => {
    ...
    console.log("⏱️ Calculate timeRange:", {
      start: startStr,
      end: endStr,
      valid: !isNaN(start.getTime()) && !isNaN(end.getTime()),
    });
...
const convertToTimeOnly = (isoString: string): string => {
  console.log("🔄 convertToTimeOnly input:", isoString);
  
  const date = new Date(isoString);
  
  if (isNaN(date.getTime())) {
    console.error("❌ Invalid ISO:", isoString);
    toast.error("Lỗi chuyển đổi thời gian!");
    return "00:00:00";
  }
  ...
  console.log("✅ convertToTimeOnly output:", result);
...
const handleSubmit = () => {
  ...
  // ✅ GỬI ISO STRING - ĐỂ normalizeSession XỬ LÝ
  const session: ResearchSession = {
    sessionId: initialSession?.sessionId,
    conferenceId,
    title: formData.title,
    description: formData.description,
    date,
    startTime: formData.selectedStartTime,  // ✅ ISO
    endTime: calculatedEndTime,              // ✅ ISO
    timeRange: formData.timeRange,
    roomId,
    roomDisplayName,
    roomNumber,
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="bg-gradient-to-br from-green-50 to-green-100 border border-green-200 rounded-lg p-3">
          ...
          <div className="text-xs text-gray-600 mt-0.5">{calculateDuration(slotStartTime, slotEndTime)}</div>
        </div>
      ...
      <div className="space-y-3">
        ...
        <p className="text-xs text-gray-500">Tối thiểu: 0.5 giờ • Tối đa: {maxTimeRange} giờ</p>
