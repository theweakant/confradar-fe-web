  ...
  export default function ResearchConferenceStepForm({
    ...
    // === LOAD EXISTING DATA ===
    const { isLoading: isConferenceLoading, isFetching, refetch } = useResearchConferenceData({
    ...
    const handleUpdateCurrentStep = useCallback(async () => {
      switch (currentStep) {
        ...
        case 2: {
          return await submitResearchDetail(researchDetail);
        }
        case 3: {
          const mainPhase = researchPhases[0];
          if (!mainPhase) {
          ...
          return await submitResearchPhase(researchPhases);
        }
        case 4: {
          if (tickets.length === 0) {
            toast.error("Vui lòng thêm ít nhất 1 loại vé!");
        ...
        case 5: {
          return await submitSessions(sessions);
        }
        case 6: {
          return await submitPolicies(policies, refundPolicies);
        }
        case 7: {
          return await submitMaterials(researchMaterials, rankingFiles, rankingReferences);
        }
        case 8: {
