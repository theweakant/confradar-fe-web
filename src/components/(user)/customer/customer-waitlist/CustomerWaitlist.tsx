...
export default function CustomerWaitlist() {
    ...
    const filterOptions = [
        { id: "most-recent", label: "Mới nhất", active: activeFilter === "most-recent" },
        { id: "start-date-desc", label: "Ngày bắt đầu (Mới nhất)", active: activeFilter === "start-date-desc" },
        { id: "available-slots", label: "Nhiều chỗ trống", active: activeFilter === "available-slots" },
    ...
    React.useEffect(() => {
        ...
        switch (activeFilter) {
            ...
            case "start-date-desc":
                sorted.sort((a, b) => new Date(b.conferenceStartDate || 0).getTime() - new Date(a.conferenceStartDate || 0).getTime());
                break;
            case "start-date-asc":
                sorted.sort((a, b) => new Date(a.conferenceStartDate || 0).getTime() - new Date(b.conferenceStartDate || 0).getTime());
                break;
            case "available-slots":
                sorted.sort((a, b) => (b.conferenceAvailableSlot || 0) - (a.conferenceAvailableSlot || 0));
                break;
            case "alphabetical":
                sorted.sort((a, b) => (a.conferenceName || "").localeCompare(b.conferenceName || "", 'vi'));
                break;
            default:
                break;
        }
