...
const ConferenceCalendar: React.FC = () => {
  ...
  const [selectedConferenceForDialog, setSelectedConferenceForDialog] =
    useState<ConferenceDetailForScheduleResponse | null>(null);
    ...
    if (calendarApi && session.startTime) {
      ...
      setTimeout(() => {
        const sessionElement = document.querySelector(
          `[data-event-id="${session.conferenceSessionId}"]`,
        );
        if (sessionElement) {
                ...
                <DialogPanel className="w-full max-w-2xl transform overflow-hidden rounded-lg bg-gray-800 border border-gray-700 p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  <div className="mb-4">
                    ...
                    <p className="text-sm text-gray-400">
                      Danh sách các phiên họp trong hội nghị. Click vào phiên
                      họp để xem trên lịch.
                    </p>
                  </div>
