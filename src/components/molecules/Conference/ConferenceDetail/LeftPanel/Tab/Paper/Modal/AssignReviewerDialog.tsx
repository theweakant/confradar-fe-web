...
interface AssignReviewerDialogProps {
  ...
  onReviewersChange: (reviewers: { userId: string; isHeadReviewer: boolean }[]) => void;
  onAssign: () => void;
  existingReviewers?: { userId: string; name: string; isHeadReviewer: boolean }[]; // Thêm prop này
}
...
export function AssignReviewerDialog({
  ...
  onReviewersChange,
  onAssign,
  existingReviewers = [], // Reviewer đã có của paper
}: AssignReviewerDialogProps) {
  // Check xem paper đã có head reviewer chưa (từ existingReviewers, KHÔNG phải selectedReviewers)
  const paperAlreadyHasHead = existingReviewers.some((r) => r.isHeadReviewer);
  ...
  // Chỉ tự động gán head reviewer nếu:
  // 1. Paper chưa có head từ trước (paperAlreadyHasHead = false)
  // 2. Người dùng đang chọn reviewer mới
  // 3. Trong selectedReviewers chưa có ai là head
  useEffect(() => {
    if (
      !paperAlreadyHasHead &&
      selectedReviewers.length > 0 &&
  ...
  }, [selectedReviewers, onReviewersChange, paperAlreadyHasHead]);
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Users className="w-5 h-5 text-blue-600" />
            {paperAlreadyHasHead ? "Gán thêm Reviewer" : "Step 1: Gán Reviewer"}
          </DialogTitle>
          <DialogDescription>
            {paperAlreadyHasHead 
              ? "Bài báo đã có Head Reviewer. Bạn chỉ có thể thêm reviewer thường."
              : "Chọn một hoặc nhiều reviewer. Chỉ một người có thể là Head Reviewer."
            }
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-5 py-4">
          ...
          {/* Thông báo nếu đã có head reviewer */}
          {paperAlreadyHasHead && (
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-blue-700">
                    Bài báo này đã có Head Reviewer ({existingReviewers.find(r => r.isHeadReviewer)?.name}). 
                    Bạn chỉ có thể thêm reviewer thường.
                  </p>
                </div>
              </div>
            </div>
          ...
          {/* Chọn Head Reviewer - CHỈ hiển thị nếu paper chưa có head từ trước */}
          {!paperAlreadyHasHead && selectedReviewerObjects.length > 0 && (
            <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Chọn Head Reviewer
              </label>
