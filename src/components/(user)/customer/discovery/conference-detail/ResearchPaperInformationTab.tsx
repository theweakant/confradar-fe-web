...
const ResearchPaperInformationTab: React.FC<ResearchPaperInformationTabProps> = ({
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold text-gray-900 mb-6">Research Paper Information</h2>

      {/* Research Phase Section */}
      {conference.researchPhase && conference.researchPhase.length > 0 && (
        <div className="mb-8">
          <h3 className="text-xl font-semibold text-gray-900 mb-4">Research Conference Timeline</h3>
          <div className="space-y-6">
            {conference.researchPhase.map((phase, phaseIndex) => (
              <div key={phase.researchConferencePhaseId} className="bg-gray-50 border border-gray-200 rounded-lg p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {phase.registrationStartDate && phase.registrationEndDate && (
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-gray-900 font-medium">Registration Period</p>
                  ...
                  {phase.fullPaperStartDate && phase.fullPaperEndDate && (
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-gray-900 font-medium">Full Paper Submission</p>
                        <p className="text-gray-600 text-sm">
                  ...
                  {phase.reviewStartDate && phase.reviewEndDate && (
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-gray-900 font-medium">Review Period</p>
                        <p className="text-gray-600 text-sm">
                  ...
                  {phase.reviseStartDate && phase.reviseEndDate && (
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-gray-900 font-medium">Revision Period</p>
                        <p className="text-gray-600 text-sm">
                  ...
                  {phase.cameraReadyStartDate && phase.cameraReadyEndDate && (
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-gray-900 font-medium">Camera Ready</p>
                        <p className="text-gray-600 text-sm">
                ...
                <div className="mt-4 pt-4 border-t border-gray-200">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <span className="text-gray-500 text-sm">Phase ID:</span>
                      <p className="text-gray-900 font-medium">{phase.researchConferencePhaseId || 'Chưa có ID phase'}</p>
                    </div>
                    <div>
                      <span className="text-gray-500 text-sm">Conference ID:</span>
                      <p className="text-gray-900 font-medium">{phase.conferenceId || 'Chưa có Conference ID'}</p>
                    </div>
                    <div>
                      <span className="text-gray-500 text-sm">Trạng thái gian đoạn:</span>
                    ...
                    <div>
                      <span className="text-gray-500 text-sm">Active Status:</span>
                      <p className="text-gray-900 font-medium">
                ...
                {phase.revisionRoundDeadlines && phase.revisionRoundDeadlines.length > 0 && (
                  <div className="mt-4">
                    <h4 className="text-gray-900 font-medium mb-2">Revision Rounds</h4>
                    <div className="space-y-2">
                      {Array.from(phase.revisionRoundDeadlines)
                        .sort((a, b) => (a.roundNumber || 0) - (b.roundNumber || 0))
                        .map((round, index) => (
                          <div key={round.revisionRoundDeadlineId || index} className="bg-white border border-gray-200 rounded p-2">
                            <div className="flex justify-between items-center">
                              <span className="text-gray-900 font-medium">Round {round.roundNumber}</span>
                              <span className="text-gray-600 text-sm">{formatDate(round.endSubmissionDate)}</span>
                            </div>
      ...
      {/* Ranking Files Section */}
      {
        conference.rankingFileUrls && conference.rankingFileUrls.length > 0 && (
          <div className="mb-8">
            <h3 className="text-xl font-semibold text-gray-900 mb-4">Ranking Documents</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {conference.rankingFileUrls.map((file, index) => (
                <div key={file.rankingFileUrlId} className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                  <div className="flex items-center gap-3">
                    <FileText className="w-8 h-8 text-blue-600" />
                    <div className="flex-1">
                      <p className="text-gray-900 font-medium">Ranking Document {index + 1}</p>
                      {file.fileUrl && (
                        <a
                          href={file.fileUrl}
                          target="_blank"
      ...
      {/* Material Downloads Section */}
      {
        conference.materialDownloads && conference.materialDownloads.length > 0 && (
          <div className="mb-8">
            <h3 className="text-xl font-semibold text-gray-900 mb-4">Material Downloads</h3>
            <div className="space-y-4">
              {conference.materialDownloads.map((material) => (
                <div key={material.materialDownloadId} className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                  <div className="flex items-center gap-3">
                    <Download className="w-6 h-6 text-green-600" />
                    <div className="flex-1">
                      <h4 className="text-gray-900 font-medium">{material.fileName || 'Download Material'}</h4>
                      {material.fileDescription && (
                        <p className="text-gray-600 text-sm mt-1">{material.fileDescription}</p>
                      )}
                      {material.fileUrl && (
                        <a
      ...
      {/* Ranking Reference URLs Section */}
      {
        conference.rankingReferenceUrls && conference.rankingReferenceUrls.length > 0 && (
          <div className="mb-8">
            <h3 className="text-xl font-semibold text-gray-900 mb-4">Ranking References</h3>
            <div className="space-y-3">
              {conference.rankingReferenceUrls.map((reference, index) => (
                <div key={reference.referenceUrlId} className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                  <div className="flex items-center gap-3">
                    <ExternalLink className="w-5 h-5 text-blue-600" />
                    <div className="flex-1">
                      <p className="text-gray-900 font-medium">Reference {index + 1}</p>
      ...
      {/* Research Sessions Section */}
      {
        conference.researchSessions && conference.researchSessions.length > 0 && (
          <div className="mb-8">
            <h3 className="text-xl font-semibold text-gray-900 mb-4">Research Sessions</h3>
            <div className="space-y-4">
              {Array.from(conference.researchSessions)
                ...
                .map((session) => (
                  <div key={session.conferenceSessionId} className="bg-gray-50 border border-gray-200 rounded-xl p-6">
                    <h4 className="text-xl font-bold text-gray-900 mb-2">{session.title || 'Chưa có tiêu đề session'}</h4>
                    {session.description && (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                      <div>
                        ...
                        <p className="text-gray-900 font-medium">{session.conferenceSessionId}</p>
                      </div>
                      <div>
                        <span className="text-gray-500 text-sm">Conference ID:</span>
                        <p className="text-gray-900 font-medium">{session.conferenceId || 'Chưa có Conference ID'}</p>
                      </div>
                      <div>
                        <span className="text-gray-500 text-sm">Room ID:</span>
                        <p className="text-gray-900 font-medium">{session.roomId || 'Chưa có Room ID'}</p>
                      </div>
                      {session.room && (
                        <div>
                          <span className="text-gray-500 text-sm">Room Info:</span>
                          <p className="text-gray-900 font-medium">
                      ...
                      <div className="mt-3">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                          {session.sessionMedia.map((media, index) => (
                            <div key={media.conferenceSessionMediaId || index} className="bg-white border border-gray-200 rounded p-2">
